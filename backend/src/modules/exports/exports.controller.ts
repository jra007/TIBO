...
export class ExportsController {
  ...
  async pdf(@Param('id') id: string, @Query('date') date: string | undefined, @Req() req: AuthenticatedRequest, @Res() res: Response) {
    const buffer = await this.exportsService.exportToPdf(id, req.user.id, date);
    res
      .set({
  ...
  async excel(@Param('id') id: string, @Query('date') date: string | undefined, @Req() req: AuthenticatedRequest, @Res() res: Response) {
    const buffer = await this.exportsService.exportToExcel(id, req.user.id, date);
    res
      .set({
